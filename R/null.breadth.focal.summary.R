null.breadth.focal.summary <- function(null.breadth.focal.out, quantiles = c(0.025, 0.975), round = 5, scaled = FALSE){
  ...
  ODB[i] <- null.breadth.focal.out[[i]]$observed.breadth
    ...
    uq[i] <- quantile(null.breadth.focal.out[[i]]$null, quantiles[2])
      ...
      if(scaled == TRUE){
        ODB <- round(ODB / scale.factor, round)
        lq <- round(lq / scale.factor, round)
        uq <- round(uq / scale.factor, round)
      }else{ODB <- round(ODB, round)
        lq <- round(lq, round)
        uq <- round(uq, round)}
